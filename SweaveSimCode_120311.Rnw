...
NewSimFun<-function(Beta_0=Beta0,
  ...
  LogModelFit<-lmer(Contacts~log(Groupsize)+(1|DyadID)+(1|PeriodID),
                      family=poisson,
                      data=modeldata)
  log.DyadR<-rpt.poisGLMM.multi(y=as.vector(ResponseMat),
                                groups=as.vector(DyadID),
                                link="log")
  log.PeriodR<-rpt.poisGLMM.multi(y=as.vector(ResponseMat),
  ...
  sqrt.beta0<-j@coefs[1]
  sqrt.theta<-j@coefs[2]
  sqrt.dyad.sd<-j@REmat[1,4]
  sqrt.period.sd<-j@REmat[2,4]
  sqrt.resid<-j@sigma
  sqrt.dyad.pt<-sqrt.DyadR$R.link
  sqrt.dyad.se<-sqrt.DyadR$se.link
  sqrt.period.pt<-sqrt.PeriodR$R.link
  sqrt.period.se<-sqrt.PeriodR$se.link
