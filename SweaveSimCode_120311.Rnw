...
#        ResponseScreen[1:dim(ResponseScreen)[1] %in% DyadsToKeep, 1:dim(ResponseScreen)[2] %in% PeriodsToKeep]<-0
#        ResponseScreen[1:dim(ResponseScreen)[1]!(%in% DyadsToKeep),!(%in% PeriodsToKeep)]<-NA
#          for(i in 1:NoDyads){
...
NewSimFun<-function(Beta_0=Beta0,Theta=Theta,Sigma_dyad=Sigma_dyad,Sigma_gamma=Sigma_gamma,ResponseScreen=ResponseScreen,NoDyads=NoDyads,NoPeriods=NoPeriods,SimGrpSize=SimGrpSize){
 ...
 DyadEffectMat<-matrix(rep(DyadEffect,times=dim(ResponseMat)[2]),nrow=length(DyadEffect),ncol=length(PeriodEffect),byrow=F)      PeriodEffectMat<-matrix(rep(PeriodEffect,times=dim(ResponseMat)[1]),nrow=length(DyadEffect),ncol=length(PeriodEffect),byrow=T)
 DyadMat<-matrix(rep(1:length(DyadEffect),times=dim(ResponseMat)[2]),nrow=length(DyadEffect),ncol=length(PeriodEffect),byrow=F)
 PeriodMat<-matrix(rep(1:length(PeriodEffect),times=dim(ResponseMat)[1]),nrow=length(DyadEffect),ncol=length(PeriodEffect),byrow=T)
      
     ResponseMat<-rpois(1,exp(Beta_0+DyadEffectMat+PeriodEffectMat))
     ResponseMat[is.na(ResponseScreen)==TRUE]<-NA  #-- test to eliminate for-loop
